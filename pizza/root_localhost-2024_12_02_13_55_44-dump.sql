...
END ;;
DELIMITER ;CREATE DEFINER=`root`@`localhost` PROCEDURE `AddHabitante`(IN label VARCHAR(255))
BEGIN
    INSERT INTO habitante (label) VALUES (label);
END ;;
DELIMITER ;
...
CREATE VIEW pizza_ingredients_with_type AS
SELECT i.id as ingredient_id, i.label as ingredient, ti.label as type
FROM ingredient i LEFT JOIN typeingredient ti on i.id_typeingredient = ti.id;