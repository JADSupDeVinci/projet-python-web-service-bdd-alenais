...
def get_random_habitante():
    ...
    if habitante:
        ...
        return jsonify({'error': 'No habitantes found'}), 404
...
def add_ingredient():
    ...
    try:
        ...
        result = db.session.execute(sql, {'label': label, 'label_commercial': label_commercial, 'id_typeingredient': id_typeingredient})
        db.session.commit()
